(() => {
  ...
  async function loadAccount(shell, attempt = 0) {
    ...
    try {
      ...
      if (!user) return;
      const { data: profile } = await client.from("profiles").select("username,coins,is_admin").eq("id", user.id).single();
      if (!profile) return;
